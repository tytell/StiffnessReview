...
```{r}
lucasdata <- read_csv('stiffness data/Lucas 2014 flexion ratio.csv')
```
...
```{r}
aleyevdata <- read_csv('stiffness data/Aleyev stiffness.csv')
```
...
From this, we can figure out the two dimensions and the second moment of area. The area $q_n = \tfrac{\pi}{4} k_n l_n$. Then
...
We cannot solve this analytically for $\kappa_n$, but we can solve it numerically. The equation is undefined when $\kappa_n = 0$ (i.e., when the body is straight), but we know that in this case $d_n = 2 \Delta s $.
