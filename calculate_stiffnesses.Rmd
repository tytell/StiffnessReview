...
From Matlab file: /Users/etytel01/Documents/2018/Whole Body lamprey/ms_plot_work_stiffness.m
...
John reports values in N m / rad, which is I think $C / \theta_{max}$.
...
EI.bass <- stiff.bass.Nmprad * len.bass.cm/100
```
...
striperskindata <- tibble(
  Lseg.mm = 50,
  E.Pa = c(152, 441, 532, 129, 112, 245),
...
displaytab <-
  ...
  mutate(across(contains('dE.'),
                ~case_when(.x == 'inc'  ~  '+',
                           ...
                           .x == 'const'  ~  '=',
                           .x == 'incdec'  ~  '+-'))) |>
  select(tissue, species, location.desc, location.L, E.Pa, EI.NL2, contains('dE.'), eta.Pas, 
         #curve.invm, strain.pct, angle.deg, 
         I.L4, 
         #measurement, value, units, 
         Lbody.mm, N, method, citenum) |>
  arrange(tissue, species) |>
  group_by(tissue) |>
  gt() |>
...
```{r}
gtsave(displaytab, 'mechtable2.docx')
```
